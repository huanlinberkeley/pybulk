    ...
    def calc(self):
        ...
        if (self.DLBIN != 0.0):
            if (self.DLBIN <= -Lnew):
                ...
                L_LLN1 = (Lnew + self.DLBIN)**-self.self.lln
        ...
        Vgd_noswap    = self.Vg - self.Vd
        Vgs_noswap    = self.Vg - self.Vs
        ...
        if (Vds < 0.0):
            sigvds = -1.0
            self.Vd = devsign * self.Vs
            self.Vs = devsign * self.Vd
        Vds  = self.Vd - self.Vs
        T0   = self.AVDSX * Vds
        ...
        q_vth   = 0.5
        T0      = self.hypsmooth((2.0 * phib + self.Vs * inv_Vt), 1.0e-3)
        nq      = 1.0 + gam / (2.0 * sqrt(T0))
        psip_th = self.hypsmooth((self.Vs * inv_Vt + 2.0 * phib + self.lln(q_vth) + 2.0 * q_vth + self.lln(2.0 * nq / gam * (2.0 * q_vth * nq / gam + 2.0 * sqrt(T0)))), 1.0e-3)
        VTH     = devsign * (VFB_i + (psip_th - self.Vs * inv_Vt) * Vt + Vt * gam * sqrt(psip_th) + Vth_shift)
        ...
        Vdssat = self.Smooth(Vdsat - self.Vs, 0.0, 1.0e-3)
        T7      = (Vds / Vdssat)**(1.0 / DELTA_t)
        T8      = (1.0 + T7)**-DELTA_t
        Vdseff  = Vds * T8
        vdeff   = (Vdseff + self.Vs) * inv_nVt
        ...
        if ((self.IGCMOD != 0) or (self.IGBMOD != 0)):
            ...
            if (self.IGBMOD != 0):
                ...
                T5     = self.lexp(T4)
                T6     = 4.97232e-7
                igbacc = self.NF * Weff * Leff * T6 * ToxRatio * self.Vg * Vaux_Igbacc * T5
                ...
                T5     = self.lexp(T4)
                T6     = 3.75956e-7
                igbinv = self.NF * Weff * Leff * T6 * ToxRatio * self.Vg * Vaux_Igbinv * T5
            ...
            if (self.IGCMOD != 0):
                ...
                igc0 = self.NF * Aechvb * T4 * (self.Vg + 0.5 * Vdsx - 0.5 * (self.Vs + self.Vd)) * igtemp
        ...
        if (self.CVMOD == 1):
            ...
            # Normalized charge qdeff at drain end of channel
            VdssatCV = self.Smooth(VdsatCV - self.Vs, 0.0, 1e-3)
            VdssatCV     = VdssatCV / self.ABULK
            T7     = (Vds / VdssatCV)**(1.0 / DELTA_t)
            T8     = (1.0 + T7)**-DELTA_t
            Vdseff = Vds * T8
            vdeff  = (Vdseff + self.Vs) * inv_Vt
        ...
        if (self.EDGEFET == 1):
            ...
            Vdsatedge = vdsatedge
            Vdsatedge = Vdsatedge + self.Vs

            # Vdssat clamped to avoid negative values during transient simulation
            Vdssate = self.Smooth(Vdsatedge - self.Vs, 0.0, 1.0e-3)
